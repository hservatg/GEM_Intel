...
!$omp target enter data map(alloc:s_counts)
    ...
    !!!which appear in declare willbe globally on the CPU and 1
    endif
...
!$omp target enter data map(alloc:r_buf)
    ...
    !write(*,*) 'me,nsize,xpsize', me, nsize, xpsize
    if( nsize .gt. xpsize) then
       write(*,*) 'PE', me, 'nsize', nsize, 'xpsize', xpsize,  'Particle array overflow'
       ierr = 1
    end if
    !    call ppsum(ierr)
...
!  Local vars
    ...
    ! Local debug vars
    real :: s_buf_sum, xp_sum

!
!  Local arrays
    INTEGER, DIMENSION(1:nvp) :: s_requ, r_requ, id_source
  !!!declare global module arrays on both CPU and 1 (htc)
...
!!$omp distribute parallel do private(i,isrt,iend,isbuf) order(concurrent) 
!! move above construct down to above the isbuf loop solved the slow kernel
    DO i=0,nvp-1
...
!s_buf_sum=0.0d0
!do i=1,s_buf_length
!  s_buf_sum=s_buf_sum+s_buf(i)
!enddo
!print *,'after 394 s_buf_length, s_buf(s_buf_length) s_buf_sum=',s_buf_length,s_buf(s_buf_length),s_buf_sum
...
!$omp target enter data map(alloc:r_buf)
   ...
   !  Check for part. array overflow
    ...
    if( nsize .gt. size(xp)) then
       write(*,*) 'PE', me, 'nsize', nsize, 'size(xp)', size(xp), 'Particle array overflow'
       ierr = 1
    end if
    !    call ppsum(ierr)
    !
...
! Code was translated using: /nfs/site/home/tkloeffe/TCE/intel-application-migration-tool-for-openacc-to-openmp/src/intel-application-migration-tool-for-openacc-to-openmp gem_pputil.F90
